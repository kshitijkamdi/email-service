import { ChevronDown, LogOut, Search, Settings, UserCircle } from 'lucide-react';
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            {addressSwitcher ? (
              <div ref={addressRef} className="relative mt-1 w-[min(22rem,calc(100vw-5.5rem))]">
                <button
                  type="button"
                  onClick={() => setAddressOpen((current) => !current)}
                  className="flex h-7 w-full items-center gap-1 text-left text-sm text-slate-500"
                  aria-label={addressSwitcher.label || 'Switch email ID'}
                >
                  <span className="min-w-0 flex-1 truncate">{addressSwitcher.value}</span>
                  <ChevronDown size={14} className="shrink-0 text-slate-400" />
                </button>

                {addressOpen ? (
                  <div className="absolute left-0 top-8 z-30 w-[min(23rem,calc(100vw-2rem))] overflow-hidden rounded-md bg-white shadow-lg ring-1 ring-line">
                    {addressSwitcher.addresses.map((address) => (
                      <button
                        key={address.email}
                        type="button"
                        onClick={() => {
                          addressSwitcher.onChange(address.email);
                          setAddressOpen(false);
                        }}
                        className={[
                          'flex w-full items-center px-3 py-2 text-left text-sm transition',
                          address.email === addressSwitcher.value ? 'bg-slate-100 font-semibold text-ink' : 'text-slate-600 hover:bg-slate-50 hover:text-ink'
                        ].join(' ')}
                      >
                        <span className="min-w-0 truncate">{address.email}</span>
                      </button>
                    ))}
                  </div>
                ) : null}
